...
public class UserService {
    ...
    public void updateUser(UserModel user) {
        if (user.getPassword() != null && !user.getPassword().isEmpty()) {
            user.setPassword(encoder.encode(user.getPassword())); // Enkodiraj lozinku ako je nova
        } else {
            // Ako lozinka nije postavljena, ne mijenjaj je
            UserModel existingUser = userRepository.findById(user.getId()).orElse(null);
            if (existingUser != null) {
                user.setPassword(existingUser.getPassword()); // Zadrži staru lozinku
            }
        }
        userRepository.save(user);
    }
